/*
   ...
   Written and maintained by Michal Zalewski <afl-wsam>
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/testWasm.wasm", out_dir);
...
EXP_ST void setup_stdio_file(void) {
  ...
  u8* fn = alloc_printf("%s/testWasm.wasm", out_dir);
...
EXP_ST void detect_file_args(char** argv) {
  ...
  while (argv[i]) {
    ...
    if (aa_loc) {
      ...
      if (!out_file)
        out_file = alloc_printf("%s/testWasm.wasm", out_dir);
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "afl-fuzz " cBRI VERSION cRST " by <afl-wsam>\n");
